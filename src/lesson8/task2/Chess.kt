...
fun bishopTrajectory(start: Square, end: Square): List<Square> =
        when (bishopMoveNumber(start, end)) {
            ...
            1 -> listOf(start, end)
                ...
                // Пересечение прямых
                val firstColumn = (end.row + end.column - start.row + start.column) / 2
                val firstRow = firstColumn + start.row - start.column
                val secondColumn = (start.row + start.column - end.row + end.column) / 2
